...
import { AlertCircle, ChevronDown, ChevronRight, Swords } from "lucide-react";
...
import CondensedPartyPathTracker from "@/components/CondensedPartyPathTracker";

export default function DesignPrototypePage() {
  const partyName = "Arden";
  ...
  const { data: allCombatCheckins } = useQuery<CombatCheckin[]>({
    queryKey: ['/api/combat-checkins'],
  });

  const { data: allCombatEncounters } = useQuery<CombatEncounter[]>({
  ...
  const handleUpdateEncounter = (id: string, updates: Partial<DBEncounter>) => {
    updateEncounterMutation.mutate({ id, updates });
  };

  const partyIssues = allIssues?.filter(i => i.party === partyName && i.status !== 'Resolved') || [];

  const combatEncountersData = (() => {
  ...
  const completedCombat = allCombatCheckins?.filter(c => c.partyId === party?.id && c.encountered).length || 0;
  ...
  if (!party || !encounters) {
    return (
      <div className="p-6">
        <div className="mb-6">
          <Badge variant="outline" className="mb-4">Design Prototype</Badge>
          <h1 className="text-3xl font-semibold mb-2">Staff Party Path - Condensed View</h1>
    ...
    <div className="p-6 space-y-6">
      <div className="mb-6">
        ...
        <h1 className={`text-3xl font-semibold ${colors.text}`} data-testid={`heading-party-${partyName}`}>
          {partyName} - Staff View
        </h1>
        <p className="text-muted-foreground">Condensed party path tracker for staff use</p>
      </div>
        ...
        <Card id="party-path-section">
          <CardHeader>
            <CollapsibleTrigger className="flex items-center justify-between w-full hover-elevate -m-6 p-6 rounded-lg" data-testid="trigger-toggle-party-path">
              <CardTitle>{partyName} - Party Path (Condensed)</CardTitle>
              {partyPathOpen ? (
                <ChevronDown className="h-5 w-5 text-muted-foreground" />
              ) : (
                <ChevronRight className="h-5 w-5 text-muted-foreground" />
